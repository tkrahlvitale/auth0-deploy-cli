...
class APIHandler {
    constructor(options) {
        ...
        this.ignoreDryRunFields = [...(options.ignoreDryRunFields || [])].filter((field, index, allFields) => allFields.indexOf(field) === index);
        this.functions = {
            list: 'list',
            create: 'create',
            delete: 'delete',
     ...
     */
    getEffectiveIgnoreDryRunFields() {
        let configured = [];
        try {
            const map = this.config('AUTH0_IGNORE_DRY_RUN_FIELDS');
        ...
        return [...this.ignoreDryRunFields, ...configured].filter((field, index, allFields) => allFields.indexOf(field) === index);
    }
    getClientFN(fn) {
        if (typeof fn === 'string') {
    ...
    getResourceName(item) {
        ...
        if (this.type === 'clientGrants') {
            const clientLabel = item._clientName || item.client_id;
        ...
        await this.client.pool
            ...
            generator: (updateItem) => retryWithExponentialBackoff(() => {
                ...
                const updatePayload = (() => {
                    ...
                    const stripped = (0, utils_1.stripObfuscatedFieldsFromPayload)(data, this.sensitiveFieldsToObfuscate);
                    return (0, utils_1.validateNoUnresolvedPlaceholders)(stripped, this.type, this.objString(updateItem));
                })();
        ...
        await this.client.pool
            ...
            generator: (createItem) => retryWithExponentialBackoff(() => {
                ...
                const createPayload = (() => {
                    ...
                    const stripped = (0, utils_1.stripObfuscatedFieldsFromPayload)(strippedPayload, this.sensitiveFieldsToObfuscate);
                    return (0, utils_1.validateNoUnresolvedPlaceholders)(stripped, this.type, this.objString(createItem));
                })();
